...
# --- Emit prompt as JSON additionalContext (TaskCompleted stdout は Claude に届かない) ---
# TaskCompleted hook は native tool で実装不完全な可能性あるが、公式仕様に準拠
emit_json() {
    python3 -c "
import json, sys
print(json.dumps({
...
if [ "$TIER" -eq 1 ]; then
    emit_json "<system-reminder severity=\"high\" action-required=\"skill-lifecycle-review\">
...
変更ファイル一覧: ~/.claude/state/implementation-checklist.pending を参照。
    ...
    emit_json "<system-reminder severity=\"medium\" action-required=\"q1-skill-check\">
