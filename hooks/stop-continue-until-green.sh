...
if blockers:
    ...
    reason = (
        ...
        "  [2] /simplify でコード品質レビュー（needs-simplify.pending 残存 + 高リスクor3ファイル以上）\n"
        "  [3] FEブラウザ検証（該当時 / Playwright MCP）\n"
        "  [4] /review で Codex 1段統合レビュー（mcp__codex__codex 1回 / または feature-dev:code-reviewer agent）\n"
        "  [5] Codex完了後 touch ~/.claude/state/codex-review.done\n"
        "  [6] rm ~/.claude/state/implementation-checklist.pending\n"
        "  [注] 1〜2ファイル/低リスクパスのみの小修正は[2][4]ともスキップ可（hookで自動判定）\n\n"
        "全完了まで作業を続けてください。\n"
        "</system-reminder>"
    )
