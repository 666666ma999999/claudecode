...
if echo "$prompt" | grep -qiE "提案|施策|見積|予算|費用|コスト|いくら|試算|効果|案を|プラン|比較|調べて|分析|指示書|まとめ|整形|清書|レポート|ランキング|優先順|順位"; then
  inject_mistake_rule "judgment-number-first" "judgment-number-first" \
    "判断を左右する導出値は 算出(式)・前提(出所)・確度 を先出し。実測値の合計・差・比率・期間換算は単位が件数/日数でも対象。数値を書く前に資料ドメインの正本の該当節を実読する（案内表= docs/response-structure-detail.md §6。広告=費用対効果連鎖式・株=catalog§0のみ・CRM=効果額に観察値注記）。施策・提案は各項目に根拠数字を添えるか、無い項目・不明ドメインは「根拠数字なし・定性判断」と明示し、実行場所（広告媒体:Meta/プロジェクト:prime_ad 等）を必ず明記。優先順位・ランキングには掲載式（並べ替え規則と各行の計算）を必ず併記する。"
