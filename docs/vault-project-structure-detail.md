...
**書式は「📎 着地点の短い段落の行末に付ける」が正**（例: `📎 承認カード「…」のリンク着地点（根拠はすぐ上） ^why-q2-deai`）
— callout/表の後ろに `^id` を独立行で置く形は Obsidian の解決が不安定で「リンクが開かない」実障害が出た（2026-07-08）。段落行末形はどのビューでも確実に解決する。
...
**グローバルルール（rules/42・vault templates/・横断 skill）を変えたセッションは、wiki/meta の該当サマリ（file-placement-rules.md 等）も同セッション内で同期する**（2026-07-09 明文化・週次監査 検証15 が mtime 突合で見張る）。
