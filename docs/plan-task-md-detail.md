...
## Phase ↔ task.md 紐付け方式（要約）

plan.md の Phase と `tasks/*.md` の紐付けは B+ ハイブリッド軽量方式。
- **命名規則**: `p<N>-<slug>.md` / `sprint<N>-<slug>.md` / `bl-<N>-<slug>.md` / `<slug>.md`
- **task.md 冒頭固定 2 行**: `**Phase:** [Phase N](../plan.md#phase-<N>)` / `**Tracker:** [phase-tracker §Phase N](./phase-tracker.md#phase-<N>)`
- **plan.md 側の安定アンカー**: 各 Phase 見出し直後に `<a id="phase-<N>"></a>` を配置（日本語見出しに依存しないリンク）

詳細（テンプレート例 / 逆リンク / 壊れリンク検知 / 規模超過時の自動化方式）: `task-progress` スキル参照。
...
## §出口 — task 完了時の退避（2026-07-15・rules/05 要約の詳細）

- 引き金: NOW.md の Done/Superseded へ 1 行集約した**同セッション**で `git mv` により `tasks/archive/` へ退避
- 移動前に必須: 機械参照の検索（`config/*.yaml` の `task_md:`・parser の glob・plan.md 参照）。機械入力/現役参照があれば archive しない
- inbound リンクは現役文書のみ張替え（歴史記録は不変）・後継ありは冒頭に後継リンク 1 行（rules/41 §出口ルールと同作法）
- 完了サマリーの正本は NOW.md Done/Superseded 表（新サマリーファイル禁止）
- 見張り: `hooks/weekly-vault-audit.sh` 検証 18（project-registry の root 駆動・Superseded=即警告 / Done=14 日未更新のみ / `task_md:` 機械参照は除外）
- 手順の実行面 = `task-progress` skill §出口

## §Red Flags（rules/05 要約の全文・2026-07-15 移設）
