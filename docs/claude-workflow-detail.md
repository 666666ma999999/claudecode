...
- **Claude × Obsidian 連携 2 セット運用 (2026-05-24〜)**:
  - **Set 1 (グローバル抽象・全 project 共通)**: Recall (decisions/mistakes 注入) / Capture (`/save decision` `/save mistake` → vault) / Overwrite (mistakes de-dup, hot.md/_index.md 完全上書き) / **Ingest (外部情報の `.raw/` 自動取得 + `wiki/sources/` 昇格・更新)** の **4 ルール**。**この節（`docs/claude-workflow-detail.md` §連携 2 セット）が 4 ルールの正本**（2026-07-16 一本化）。実装規約は `rules/40-obsidian.md`。vault 側の `wiki/concepts/Claude-Obsidian feedback loop.md` / [[vault-rules-global]] / [[vault-rules-project]] は**本節の表示用ミラー**（drift 時は本節が優先・vault 側で正本を再定義しない）
