...
- **`tasks/NOW.md` への書込は行わない**（2026-07-11 team-lead 裁定: 実体が MASA.local に存在せず masa-2 側が正本の可能性があるため、自動書込対象から外し「手動確認」に留める。Q番号→M番号の自動対応表も作らない）
...
- interventions.csv 正本候補: `~/Desktop/prm/prime_suite/prime_ad/metrics/interventions.csv`（2026-07時点で MASA.local には実在しない＝masa-2側が正本。2026-07-21 に MASA.local で不在を実確認）
...
2. 運用する場合は、実行するホストを問わず、まず `--apply` なしで dry-run 出力を目視確認し、抽出された裁定と追記内容が想定通りであることを確認してから `--apply` を付ける
