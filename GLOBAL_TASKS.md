...
**背景**: 
- エクステンション設計ルール（旧 `~/.claude/rules/60-cms-and-extension-pattern.md`・削除済 → 現 `be-extension-pattern` / `fe-extension-pattern` / `fe-be-extension-coordination` スキル + `rules/30-routing.md` の分岐表）はマーカーファイル検出方式（`config/extensions.yaml` or `config/extensions.json`）で適用判定する設計
...
2. extension化しないプロジェクトは `~/.claude/rules/70-architecture.md`（FE/BE 統合）のみ適用
3. art_012/013 の extension化セクションは「実体験」ではなく「一般ガイド」として位置づけを明記するか、削除

**参照**:
- `be-extension-pattern` / `fe-extension-pattern` / `fe-be-extension-coordination` スキル（旧 `rules/60-cms-and-extension-pattern.md` は削除済・後継）
