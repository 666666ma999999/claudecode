...
## coverage-floor ゲート（2026-07-15 裁定・出典: totty2 metrics-3layer §2・Fable5×Sol レビュー F8）

集計・監視で率/分位点を出すとき、**「全て clean（違反0）」と「そもそも0件で判定できない（判定不能）」を必ず区別する**。空データを 0.0 で表示すると監視が沈黙し、悪化を見逃す。

1. 分母 < `MIN_DENOMINATOR`（既定 30）、または対象イベント 0 件のとき、率を **0.0 と表示せず** `{"indeterminate": true, "n": …}` を返す
2. 監視バッチ（daily_monitor 型・「read-only 集計→非ゼロ exit」パターン）は、違反>0 だけでなく **coverage 失敗（0セッション/0サンプル/欠損率超）でも非ゼロ exit** にする（正常終了で沈黙させない）
3. 参照実装: `backend/tools/report_ops_metrics.py`（判定不能規約）・`backend/tools/daily_monitor.py`（strict 監視）
