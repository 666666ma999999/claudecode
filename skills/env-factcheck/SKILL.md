...
description: >-
  ...
  アーカイブ用スクリプト（手動実行・スケジューラ未配線）: `~/.claude/state/weekly-metrics-archive.sh`（JSONL 消失前のスナップショット永続化。retention は settings.json `cleanupPeriodDays`＝現行365日、`~/.claude/metrics/weekly/YYYY-Www.json`）。
