...
上記を全て実施後:
1. **決定事項の反映照合（surface-compliance 対策・必須）**: 同セッションでユーザーが決定・訂正した事項を列挙し、成果物への反映を 1 件ずつ「決定 → 反映先ファイル:行 or 未反映」で照合した表を報告に含める。観測（実ファイル確認）を伴わない「反映済み」宣言は禁止（recurring-mistakes `obs-before-claim`）。
