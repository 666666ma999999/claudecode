...
**STEP実行レベルの自動リトライは禁止。** エラー発生時は即停止し、ユーザーに報告して判断を仰ぐこと。

### バックエンド内部の低レベルリトライ（許可）

バックエンド内部のネットワーク接続レベルのリトライのみ許可する。
...
- これらはバックエンドAPI内部で処理される低レベルのネットワークリトライであり、STEP再実行ではない
- リトライ回数はセッションストレージで追跡される（`loop_counters`内）
- 非HTTPの例外はトレースバックが自動キャプチャされ、セッションJSONに記録される（デバッグ用）
- 低レベルリトライが上限に達した場合、即停止してユーザーに通知する
