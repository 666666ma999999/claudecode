...
| 6 検証 | — | /review + security-twin-audit(差分指定) + checklist | **既存環境** |
...
2. **ExitPlanMode** 後、SubAgent 委託ルールに従い実装
   - 変更2ファイル以上 → SubAgent 必須（最低 2: Explore + Verify。アーキ変更は +Implement）
   - バッチ検証ループ: 3編集ごとに中間検証
   - Docker-Only 開発ルール適用
...
1. `/review` — Codex コードレビュー（1段統合: 仕様準拠 + コード品質を 1 プロンプトで実行）
2. `security-twin-audit`（差分指定）— セキュリティ差分監査（対象条件に該当する場合）
