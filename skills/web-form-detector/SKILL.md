...
// 複合条件
await page.click('button.primary:has-text("確定")');
...
```javascript
// テキスト入力
await page.fill('input[name="title"]', '入力値');

// クリア＆入力
await page.fill('input', '');  // クリア
await page.fill('input', '新しい値');

// 1文字ずつ入力（キーイベント発火）
await page.type('input', 'テキスト', { delay: 100 });
```
...
```javascript
// ドロップダウン選択
await page.selectOption('select', { label: '選択肢1' });
await page.selectOption('select', { value: 'option1' });
await page.selectOption('select', { index: 2 });

// 複数選択
await page.selectOption('select[multiple]', ['opt1', 'opt2']);
...
```javascript
// 直接セット
await page.setInputFiles('input[type="file"]', '/path/to/file.csv');

// 複数ファイル
await page.setInputFiles('input[type="file"]', ['/path/file1.csv', '/path/file2.csv']);

// ファイルチューザー対応
const [fileChooser] = await Promise.all([
  page.waitForEvent('filechooser'),
  page.click('button:has-text("ファイル選択")')
]);
