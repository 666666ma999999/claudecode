...
`tasks/<task-name>.md`に以下の形式で書き出す（タスク名はGoalから導出）:
...
## Execution Topology
- 規模: [標準 / 大規模]
- SubAgent構成: [Explore + Verify / Explore + Implement + Verify]
- Main Agentの責務: [統合・意思決定・接着実装の概要]

## 実装計画

### Batch 1: [フェーズ名（例: データモデル追加）]
- [ ] T1: [動詞+対象] | `ファイルパス` | owner: [Main/Implement] | 依存: なし
- [ ] T2: [動詞+対象] | `ファイルパス` | owner: [Main/Implement] | 依存: T1
- **fast_verify**: [このバッチの最短検証コマンド（例: pytest tests/test_models.py -x）]

### Batch 2: [フェーズ名（例: API実装）]
- [ ] T3: [動詞+対象] | `ファイルパス` | owner: [Main/Implement] | 依存: T1
- **fast_verify**: [このバッチの最短検証コマンド]

## Final Verification
- [ ] [全体の最終検証（例: 全テスト実行、E2Eテスト、ブラウザ動作確認）]

## 中断時の状態
- 次のアクション: Batch 1のT1から開始
- 未解決問題: なし
```
