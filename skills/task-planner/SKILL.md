...
`tasks/<task-name>.md` に書き出す（タスク名は Goal から導出）。**フォーマットの正本 = `~/.claude/templates/task.md`（1 セッション完結なら `task-light.md`）** — 本 skill 独自のフォーマットは持たない（正本 2 択と矛盾していた旧・独自テンプレは 2026-07-11 棚卸し P2 裁定で削除）。

テンプレの必須セクション（成功基準・Progress・Session Handoff 等）に加えて、Phase 4 で作った内容を次の対応で埋める:

- 成功基準 → テンプレ `## 成功基準` へ
- 規模・SubAgent 構成（Explore + Verify 等）・Main の責務 → テンプレの Scope/Progress 冒頭に 1 行ずつ
- Batch 分割（各 Batch に T# タスク行 `[動詞+対象] | ファイルパス | owner | 依存` と **fast_verify** コマンド）→ テンプレの Progress 配下に `### Batch N` として記載
- 最終検証 → テンプレの検証欄へ／中断時の再開点 → `## Session Handoff` の Start Here へ
