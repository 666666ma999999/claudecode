...
## 10-git-and-execution-guard.md / refactoring-guide との関係

- `rules/10-git-and-execution-guard.md` は実行統制の上位ルール。ブロッカー時は即停止・確認を優先
...
6. 失敗時は `rules/10-git-and-execution-guard.md` のブロッカープロトコルで停止する
...
**削除だけでなく「変更」も横断する**: ①SSoT 定数をモード対応・変更するときは rg で**全参照層**（特に独立した品質ゲート・検証層）を列挙し、全層に同じ分岐を通す（一部層のみ対応→自己矛盾が実例） ②前提を変更したら旧前提キーワードを全文 grep → 全箇所を失効/更新 → 全文通読 ③型・gateway で ownership を保証していても direct assignment を grep audit ④**ファイル/パス/名前の rename 時**は bin/ hooks/ rules/ skills/ docs/ を横断 grep して dead reference を同一セッションで追従（後日修理が 5 回反復した実測）。
