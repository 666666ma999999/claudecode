...
description: |
  ...
  NOT for: 単純な構文エラー、初回のデバッグ試行、SubAgent委託判断（→ execution-patterns）、テストフレームワーク固有の修正（→ test-fixing）
...
## 事実確認ルール（CLAUDE.md連携 — 2層目）

デバッグ・トラブルシュートの全フェーズで以下を徹底:
- **推測で原因を断定しない**。ツール（curl, API呼び出し, ログ確認）で実態確認してから診断する
- **外部サービスのエラー**: エラーコード → 公式ドキュメント照合 → 実際のレスポンス確認 → 診断
- **ユーザーの契約形態・利用実績を勝手に推測しない**。確認できない情報は「未確認」と明示する
- **確認不可の場合**: 推測で埋めず、ユーザーに確認手段を提示する
...
1. **即座に修正作業を停止**
2. エラー報告フォーマット（CLAUDE.md「エラー報告・バグ修正」節: 症状 → 原因（ファイル:行番号）→ 選択肢2つ以上）でユーザーに報告
3. 以下を含める:
   - 試した3つの修正とその結果
   - 現在の仮説とその限界
   - アーキテクチャ見直しの要否
...
5. **複数コンポーネント系の証拠収集**: CI→ビルド→署名、API→サービス→DB のように複数コンポーネントをまたぐ場合、修正に入る前に各コンポーネント境界で入力・出力・環境伝播をログし、一度の実行で「どの層で壊れているか」を可視化してから、その層を深掘りする
<!-- 出典: obra/superpowers systematic-debugging @ c8921b5 (MIT)・2026-07-24 一部取込 -->
...
| 「成功してるのに反映されない」 | — | 非ブロッキング後処理の WARNING/skip ログと認証失効を最初に見る |
| エラーの握り潰し（2>/dev/null / \|\| true） | — | 空・失敗・parse失敗を区別。無人ジョブは成果物 age の鮮度ガードを置く |
| fallback 付き外部 API の失敗帰属 | — | 実際に採用された provider をログで裏取りしてから成否を帰属する |
| 稼働プロセスの状態を別プロセスで直書き | メモリキャッシュが旧版を書き戻す | プロセス経由 API で編集（直書き→再読込確認は偽陽性） |
| 「緊急だから手順を省略」 | 場当たり対応は結局手戻りを生み、体系的デバッグより遅くなる | 緊急時こそ Phase 1 から通す |
| 「今回だけ当てずっぽうで試す」 | 3回目以降の失敗はアーキ問題のシグナルであり、当てずっぽうは的中しない | 3-Fix Limit に従い、仮説明文化（Phase 3）に戻る |
<!-- 出典: obra/superpowers systematic-debugging @ c8921b5 (MIT)・2026-07-24 一部取込 -->
