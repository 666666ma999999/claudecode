...
①N=1 サンプルから skill/hook/自己改善ループを scaffold しない（手動で 2-3 回回してから） ②「集めて/作って」系依頼は着手前に「作った後に何を決めたいか」を 1 問確認。敵対レビューが「過剰」と言ったら最小で止める（collect-reply-posts が実運用 0 件の失敗例）。

**スキル新設・改名時の定型**: `30-routing.md` のトリガー行追加と近隣スキルへの NOT-for 相互注記を**同一 commit** で行う（後追い commit が 3 回以上反復した実測）。
