...
3. 各小見出しセクションのCSSクラスがrohan登録komi_typeと一致するか確認（`free-komi-type`/`paid-komi-type`独立チェック項目で実施、上記変換ルール適用）
...
Sentry MCPが有効な場合、本番チェックに以下を追加：

| # | 項目 | 確認内容 | 条件 |
|---|------|---------|------|
| 24 | 未解決Sentryエラー | 対象プロジェクトの未解決issueが0件 | Sentry MCP有効時のみ |

### 実行方法
1. Sentry MCPの `list_issues` で未解決issueを取得
2. 0件 → チェック通過
3. 1件以上 → 信頼度を1段階下げ、issue一覧を報告

### Sentry未導入時
- このチェック項目はスキップされる（既存23項目に影響なし）
