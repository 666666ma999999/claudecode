...
**悪い実例（修理済み・パターン参照用）**: `hooks/stop-dup-guard.sh` は見出し重複検知が fenced code block を除外しておらず、2026-07-05 の監査セッションで同一メッセージに5回連続誤発火した（ブロック重複検知は `in_fence` で除外済みなのに見出し側だけ未対応という非対称が原因）。同日 `46b09e9` で見出し側にも `_in_fence` 除外を追加し修理済み（現 L92-105）。新規 hook では両検知パスに最初から同じ fence 除外を入れること。
...
- **悪い例（修理済み・46b09e9）**: `hooks/stop-dup-guard.sh` は当時、見出し重複検知が fenced code block を除外しておらず、テンプレ例示を二重記載と誤認。2026-07-05 に同一メッセージへ 5 回連続誤発火（②の反面教師。同ファイル L110-118 のブロック検知は fence 除外できていたのに見出し側だけ未対応、という非対称が実害の核）。同日 `46b09e9` で見出し側にも fence 除外を追加し修理済み。
...
- 修理コミット 8 本（`git log -1 <hash>` で照合可）: `c6c67eb3`（headless で Stop hook 3 本無効化）/ `c9ee0588`（evidence-footer 誤検知修正）/ `9ccf8bef`（verify-step の session スコープ化）/ `aaa51127`（追記ログの retention cap）/ `840f825c`（_profile-wrapper 撤去）/ `65c0fd95`（restrict-cwd-edits の worktree allow）/ `0e3b7f8f`（act-time ガード 3 本新設）/ `46b09e9`（stop-dup-guard.sh 見出しfence除外の修理）
