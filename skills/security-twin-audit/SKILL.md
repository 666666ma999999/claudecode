...
> ⚠️ 以下の `TeamCreate` / `Task tool` 記述は旧ツール仕様（TeamCreate は廃止・Task は Agent へ改称済み 2026-07-10 確認）。現行環境ではこの手順のままでは実行できないため、**「Agent Teams なしでの実行（簡易版）」節を現行の主経路として使うこと**。本節の書き直しは提案中（監査レポート 2026-07-10 参照）。
