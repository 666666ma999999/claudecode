...
1. マーカーファイル検出でプロジェクト種別を判別:
   ...
   - 両方あり → ハイブリッド (`rules/70-architecture.md` 適用)
   - なし → 汎用プロジェクト
...
2. Obsidian NOW→DONE 移動は `/done` コマンドに従う（skill は _dormant）。
3. `tasks/{slug}.md` に Session Handoff セクションを追記・更新する（テンプレ `templates/plan.md` には同セクションが無いため新設。書式は `task-progress` スキル参照）。
