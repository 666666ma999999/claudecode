...
| ランキング比較（項目間の大小） | 棒グラフ | `xychart-beta` + `bar` | 商品別 平均金額・CP別 CV |
| 量の比較（現状↔目標・前後・A/B） | 棒グラフ(系列) / 2列表 | `xychart-beta` + `bar` | 現状CV vs 目標CV・before/after |
| 時系列の伸び（推移・トレンド） | 棒 / 折れ線 | `xychart-beta` + `bar`/`line` | 会員数の伸び・段階別CV/予算推移 |
| 手順・分岐・連鎖（プロセス） | フローチャート | `flowchart LR/TD` | ロードマップ・判定フロー・attack narrative |
...
> ラベル付きエッジの連結 `A -->|x| B -->|y| C` はパースエラーで非表示 → エッジは 1 本ずつ改行する。`xychart-beta` は新しめの Mermaid (10.3+) 要・棒と折れ線を同一チャートに併置可 (`bar [...]` + `line [...]`)。
