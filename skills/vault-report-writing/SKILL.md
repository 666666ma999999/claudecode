...
- ユーザーが読む成果物（依頼書・報告書・findings・**blind read パケット・読み比べ資料・判定依頼資料・チェックリスト等、ユーザーの目と判断を要するすべての md**）は**最初から vault に作成**する（frontmatter + MOC 索引行つき）。repo docs/・tasks/ は機械が読む仕様・実行追跡のみ。ユーザーは repo を見ない（invisible-SSoT・2026-07-14 に読み比べパケットで同型再発=「バルトMDって何回言えばいい？」・成果物の種類名で判定せず「ユーザーが読むか」で判定すること）。
...
- 定期実行で自動生成し他ノートから embed されるレポートは、embed 対象の見出しを改名しない（埋め込みが切れる）（出典: AIads-playbook §2）。

## 仕上げチェックリスト

- [ ] **（施策・提案を含む全レポート）各施策に「実行場所」を明記したか** — 広告なら媒体・配信面（例: 広告媒体:Meta）、作業ならプロジェクト・対象ファイル（例: プロジェクト:prime_ad）。場所のない提案は実行できない（2026-07-24 ユーザー恒久指示・初出=パワーワード台帳で媒体欄の欠落を指摘された再発防止）
...
- [ ] **（複数システム/媒体が1枚に同居するボードのみ）全数字・窓・施策に【媒体】ラベルと計測範囲（期間）を明記したか・窓を跨いだ比較になっていないか**（出典: AIads-playbook §1b/§5d・2026-07-08）
