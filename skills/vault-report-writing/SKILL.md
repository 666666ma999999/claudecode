...
- [ ] **（司令塔型のみ）状態の地図（重症度降順）と実行順を別表にし NOW# で相互参照**したか・**full**（サマリ+優先ロジック+やること内包）で薄い索引化していないか・自動生成ゾーンを最下段に置いたか・**🎯を時間軸3層（🔴きょう/🟡今週/🟢中長期=Phase）で持ったか**・**図解2種（時間軸プロセス図／システム設計図+置き場所パス併記・skeleton §4）を検討したか**（2026-07-06 要件） 
