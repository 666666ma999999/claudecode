...
description: |
  ...
  NOT for: 経営資料・行政資料・KPI監視・役員向けダッシュボード（→ dashboard-design-guide）
...
| 負値あり かつ 歪度の絶対値 < 1.0 | DIVERGING | ゼロ中心の正負分布 |
...
| 負値比率 > 30% | RdYlGn | 良い/悪いの直感的表現 |
| 極値方向 up（非diverging時） | Greens | 上昇トレンド強調 |
| 極値方向 down（非diverging時） | Reds | 下落トレンド強調 |
...
│   └── is_growth_rate → WATERFALL（変化の積み上げ・※scripts の pipeline は未実装。references/chart-templates.md の render_waterfall を手動選択）
...
```python
X_POST_CONFIG = {
    "figsize": (12, 7),           # dpi150で1800x1050px出力（X推奨1200x675と同系の横長比）
    "dpi": 150,                    # 高解像度
