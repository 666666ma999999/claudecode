...
前提は既定として明示して進め、結論を変える未承認の仮定がある時だけ1回確認する（毎回「正しいですか」で止めない）。施策・提案は**各項目ごと**に根拠数字を最低1つ（実測 or 式付き見積り）添え、無い項目は**その項目内に**「**根拠数字なし・定性判断**」と明示する。
対象は判断駆動数値だけ（実測値の単純提示・日付・件数・バージョン・保存KPIの引用は対象外。**ただし実測値からAIが計算した合計・差・比率・期間換算は、結果の単位が件数/日数でも対象**）。全文 → `docs/response-structure-detail.md §6`。
