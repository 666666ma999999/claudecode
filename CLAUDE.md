...
- 単純な構文確認・API仕様確認は `/btw` を使い、コンテキストを消費しない
...
- NOW完了時:
  - 完了タスクをNOW→DONEに移動。DONEはカテゴリ別に振り分け（機能追加/バグ修正/UI/リファクタリング/調査 等、内容から判定）、末尾に完了日を付ける。元の指示プロンプトはそのまま残す（例: `- スプレッドシートから商品を一括登録 / ロジック:数秘相性 / まず1商品テスト (2026-04-09)`）
