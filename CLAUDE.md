...
次の条件を**1つでも**満たしたら、**次のユーザー応答前**に `implementation-checklist` スキルを必ず実行する:

- `Write/Edit` で実行コード（Python/JS/HTML/CSS）を変更した
- 設定変更により挙動が変わりうる（`.mcp.json`, `config.py` 等）
- ユーザーへ完了報告・修正報告・確認依頼・レビュー依頼を返す

**免除にならないもの:**
- 「まだ完了ではない」「一旦確認してください」— 途中報告でも免除しない
- デバッグ中の反復修正 — 各修正後に検証を実施してからユーザーに返す
- 例外判定はタスク全体のdiffで行う。パッチ単位で「1行だけ」と判定しない

**除外（タスク全体で以下のみの場合）:** ドキュメントのみ、コメントのみ
