...
> **改訂履歴**: 2026-05-16 まで「strategy/progress/measures + MOC の 4 ファイル必須」だったが、vault と repo の二重管理 (vault progress.md ↔ repo phase-tracker.md 等) が判明し、2026-05-17 に MOC 1 ファイル統合に純化。2026-05-18 に「subproject ごとに 1 ファイルしか持たない場合はサブディレクトリを切らず group 直下に直置き」に再純化 (`AI_adscrm/AIads/AIads_ope.md` 等の 1 ファイル 1 ディレクトリ廃止)。
...
- **registry の置き場所は `02_Ai/AI_adscrm/project-registry.md` に固定** (hook `~/.claude/hooks/sessionstart-project-registry.sh:26` で hardcode・他 group は registry を持たず、同一 registry に追記)
...
検証: `weekly-vault-audit.sh` は MOC 1 ファイル前提で動作・他 vault note は任意

違反の典型 (5/8 同型・要警戒):
- strategy.md に `## repo 側正本` 表が独立で存在 (MOC との重複)
- strategy.md の Phase 別索引に絵文字状態 (`🟢 投入準備済` 等) が直書き
- measures.md の H2 (`## 🟢 Phase 0A (進行中・〜2026-05-15)`) に期間・状態を含む

検証: `weekly-vault-audit.sh` への将来拡張で機械検出可能 (現状は手動 review)。
...
**成功基準（grep 検証可能）**:
```bash
# 新プロジェクト作成後に全て PASS すること

# 1. MOC ハブ存在
ls 02_Ai/<project>/ | grep "_ope.md"

# 2. frontmatter 6 必須フィールド全検証 (例外 type を除く)
for f in 02_Ai/<project>/*.md 02_Ai/<project>/<sub>/*.md; do
  # 例外 type (concept/registry/guide) はスキップ
  if grep -qE '^type: (concept|registry|guide)$' "$f"; then continue; fi
  for key in 'project:' 'type:' 'folder:' 'categories:' 'last_updated:' 'tags:'; do
    grep -q "^$key" "$f" || echo "❌ $f: $key 欠落"
  done
done

# 3. Phase 正本存在 (subproject の progress.md)
grep -l "## Phase 進捗" "02_Ai/<project>/<sub>/<scope> progress.md"

# 4. wikilink ambiguity 検出 (vault 全体で同名重複なし)
for name in plan.md measures.md progress.md index.md strategy.md hub.md; do
  count=$(find ~/Documents/Obsidian\ Vault -name "$name" | wc -l)
  [ "$count" -le 1 ] || echo "❌ $name: $count 件重複"
done
```
...
`40-obsidian.md` > 本ルール > 各スキル SKILL.md。競合時は `40-obsidian.md` 優先。
