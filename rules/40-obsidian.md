...
| `ingest <file\|url>` | claude-obsidian | `References/raw/` → `wiki/` | `skills/wiki-ingest/SKILL.md` |
...
vault 全域 CRUD 可能なため、不変ルール（既存 142 件無変更 / `References/raw` `refs/` append-only）を一発で破る危険がある。以下を厳守:

- **許可される操作**: `wiki/**` 配下の作成・更新、新規 `.base` / `.canvas` ファイル作成
- **禁止される操作**: vault 直下の既存 142 件 md / `References/raw/` 既存ファイル / `<project>/refs/` 既存ファイルの **更新・削除**
...
| PostToolUse | Write\|Edit | `wiki/` `References/raw/` を auto-commit | `[ -d wiki ] && [ -d .git ]` |
...
- `References/raw/` は **append-only**。過去ソースを書き換えない
...
- vault 外プロジェクトで `wiki/` `References/raw/` への書き込みが発生している
- hook が `[ -d wiki ]` ガードなしで vault 操作している
- 既存 142 ノートに git diff が出ている
- `References/raw/` 配下のファイルが書き換えられている（append-only 違反）
- `obsidian-cli` が `/save` `/canvas` `/autoresearch` `/wiki-ingest` などの workflow skill から自動呼び出しされている
- vault 直下既存 142 件 md / `References/raw/` 既存ファイル / `refs/` 既存ファイルが `obsidian-cli` 経由で書き換えられている
