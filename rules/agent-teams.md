...
SubAgentまたはTeammateによる実装後、以下の2段階でレビューを行うことを推奨する:

**Stage 1: 仕様準拠レビュー**
- 実装がタスクの仕様・要件を満たしているか確認
- 入出力が期待通りか、エッジケースが処理されているか
- 担当: 仕様を理解しているレビュアー（リーダーまたは専任Teammate）

**Stage 2: コード品質レビュー**
- コーディング規約への準拠（coding-standards参照）
- パフォーマンス・セキュリティの懸念
- テストの十分性
- 担当: コード品質に精通したレビュアー
...
- 軽微な指摘（typo等）はまとめて修正可。設計レベルの指摘は即対応
