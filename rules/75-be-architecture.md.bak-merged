...
1. `20-code-quality.md` の変更時ゲート（横断検索・正規モジュール特定・既存経路拡張確認）を実施したか
2. 既存pipeline拡張で対応可能か検討したか
3. 追加後に旧経路との責務重複がないか確認したか

条件を満たせない場合は `10-git-and-execution-guard.md` のブロッカープロトコルに従い停止し、ユーザーに確認する。

## Red Flags（BE固有 — 汎用Red Flagsは `20-code-quality.md` を参照）
