...
### トリガー条件（以下のいずれかに該当したら即座に発火）

| # | 条件 | 例 |
|---|------|-----|
| 1 | Edit/Writeツールでコードファイルを変更した後、次にユーザーへ報告する時点 | バグ修正、機能追加、リファクタリング完了時 |
| 2 | SubAgentがコードファイルを変更して完了報告を返した時点 | Teammate/SubAgentによる実装完了 |
| 3 | ユーザーが「実装完了」「修正完了」「できた」等の完了を示す発言をした時点 | 確認依頼 |
| 4 | `/verify-step` コマンド実行時 | 明示的な発火 |

### 発火アクション

トリガー条件に該当したら、**Skillツールで `implementation-checklist` を呼び出す**こと。
ユーザーへの完了報告・次タスクへの移行前にスキップ禁止。

### 除外条件（発火不要）

- ドキュメントのみの変更（MEMORY.md、CLAUDE.md、SKILL.md、README.md）
- 設定ファイルのみの変更（settings.json、.claude/配下）
- 調査・分析のみ（コード変更なし）
- 1行以下の軽微な修正（typo、コメント修正）

### 自己チェック

ユーザーへ完了報告を書く直前に、以下を確認すること：
「今回の作業でEdit/Writeツールでコードファイルを変更したか？ → YES → implementation-checklistを実行済みか？ → NO → 今すぐ実行」
