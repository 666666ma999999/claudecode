...
ツール選択の詳細は `web-tool-selector.md` を参照。ここでは実装パターンを記載。

### Level 0: WebFetch（静的HTML）

```
# 最もシンプル・高速。静的HTMLで十分な場合に使用
WebFetch(url="https://example.com/page", prompt="ページの価格情報を抽出")
```
...
```
# actionsで簡単なページ操作を実行してからスクレイピング
firecrawl_scrape(
...
Firecrawl MCPが利用可能かどうかを事前に確認し、未導入環境でもグレースフルに動作させる。
...
### Firecrawlキャッシュ（maxAge）

同じページを短時間に複数回取得する場合、キャッシュで重複リクエストを回避:
...
firecrawl_scrape(
  ...
  formats=["markdown"],
  # キャッシュ有効期間（秒）
  # 同一URLへの再リクエストがこの期間内ならキャッシュから返却
)
```
