...
### 非適用プロジェクト（マーカーファイル不在で確認済み）

以下のプロジェクトはエクステンションアーキテクチャを採用していない。従来のディレクトリ構造（`backend/routers/`, `frontend/` 等）をそのまま維持すること:
- `~/Desktop/prm/rohan` — STEP 1-8パイプライン型ワークフロー（StepHandler Registry方式）
